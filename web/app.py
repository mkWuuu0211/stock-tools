...
if tool == "🏠 首页":
    ...
    https://github.com/你的用户名/stock-tools
    """)
    ...
    def init_services():
        ...
        return dm, pattern_service, sync_service


    dm, pattern_service, sync_service = init_services()
                    ...
                    if results is None or len(results) == 0:
                        ...
                        for i, result in enumerate(results[:10]):
                            with st.expander(f"#{i+1} {result['ts_code']} {result['name']} (相似度: {result['score']:.3f})", expanded=i < 3):
                                ...
                                # 目标
                                norm_target = (target_prices - target_prices.min()) / (target_prices.max() - target_prices.min())
                                fig.add_trace(
                                    go.Scatter(
                                ...
                                norm_match = (match_prices - match_prices.min()) / (match_prices.max() - match_prices.min())
                                fig.add_trace(
                                    go.Scatter(
